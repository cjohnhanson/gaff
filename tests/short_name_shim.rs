//! The short-name binary is what `uvx gaffr` and the .deb install, and
//! it execs `gaff` beside it. No test ran it, so a wrong sibling name
...
fn the_shim_answers_as_the_long_name_does() {
    let (short, short_code) = run(env!("CARGO_BIN_EXE_gaffr"), &["--version"]);
    let (long, long_code) = run(env!("CARGO_BIN_EXE_gaff"), &["--version"]);
    assert_eq!(short_code, long_code);
    assert_eq!(short, long, "the shim should print what gaff prints");
...
fn the_shim_passes_every_argument_through() {
    ...
    let (short, _) = run(env!("CARGO_BIN_EXE_gaffr"), &["docs", "configuration"]);
    let (long, _) = run(env!("CARGO_BIN_EXE_gaff"), &["docs", "configuration"]);
