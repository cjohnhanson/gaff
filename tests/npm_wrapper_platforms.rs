...
//!
//! The wrapper is the entry point for `npx gaffr`, and no Rust test
...
fn the_wrapper_loads_without_throwing() {
    ...
    let out = std::process::Command::new("node")
        .args(["-e", "require('./npm/gaffr/bin/run.js')"])
        .output()
        .expect("node runs");
...
fn the_refusal_names_the_crate_not_the_package() {
    ...
    let out = run_for("sunos", "x64");
    assert!(
        out.contains("cargo install gaff"),
...
fn run_with_binary(path: &str) -> (String, i32) {
    let out = Command::new("node")
        .args(["npm/gaffr/bin/run.js"])
        .env("GAFFR_BINARY", path)
        .output()
        .expect("node runs");
    (
        format!(
...
fn a_binary_killed_by_a_signal_does_not_exit_zero() {
    ...
    let dir = std::env::temp_dir().join("gaffr_signal_probe");
    std::fs::create_dir_all(&dir).expect("temp dir");
    let script = dir.join("gaff");
...
fn referenced_package_names() -> Vec<String> {
    let src = std::fs::read_to_string("npm/gaffr/bin/run.js").expect("wrapper");
