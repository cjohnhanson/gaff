...
fn every_npm_publish_path_is_relative() {
    ...
    // reads it as a folder. `npm publish npm/gaffr-darwin-arm64` cloned
    // `github.com/npm/gaffr-darwin-arm64` and failed on a key it has no
    // reason to hold, so a tagged release reached three registries and
    // published nothing to npm.
    let mut checked = 0;
    for line in workflow().lines() {
        let trimmed = line.trim();
        let Some(rest) = trimmed.strip_prefix("npm publish ") else {
            continue;
        };
        let path = rest
            .split_whitespace()
