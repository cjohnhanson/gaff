...
fn report_github_problems(cfg: &config::Config) -> Option<ExitCode> {
    ...
    for wf in cfg.github.iter().filter(|w| w.user) {
        eprintln!(
            "gaff: the workflow `{}` comes from your user config, so it renders into whatever repo you run this in, and the file it writes is not reproducible from that repo alone. Declare it in the repo's config instead.",
            wf.name
        );
    }
