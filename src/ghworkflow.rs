...
impl Workflow {
    ...
    pub fn problems(&self, git: &[crate::githook::GitHook]) -> Vec<String> {
        ...
        if self.name.is_empty() || self.name.contains('/') || self.name.contains('.') {
            out.push(format!(
                "workflow `{}`: the name becomes a filename, so it must hold no dot or slash",
                self.name
            ));
        }
        if self.on.is_empty() {
...
mod tests {
    ...
    fn an_argument_that_needs_quoting_gets_it() {
        ...
        let out = render(&w, &[]);
        assert!(out.contains("run: sh -c 'echo hi && exit 0'"), "{out}");
    }
    ...
    fn an_unknown_event_and_a_bad_name_are_errors() {
        ...
        assert!(
            problems.iter().any(|p| p.contains("no dot or slash")),
            "{problems:?}"
        );
        assert!(
