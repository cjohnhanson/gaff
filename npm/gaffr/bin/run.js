...
const rel = env.GAFFR_BINARY ? null : PLATFORMS?.[key]?.[arch];
const bin = env.GAFFR_BINARY || (rel && require.resolve(rel));

if (!bin) {
  console.error(
    `gaffr ships no prebuilt binary for ${platform} ${arch}. ` +
      "Install it with `cargo install gaffr`, or set GAFFR_BINARY to a path."
  );
  process.exitCode = 1;
} else {
